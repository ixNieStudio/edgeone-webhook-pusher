  ...
  <div class="h-full flex flex-col">
    ...
    >
      <div class="flex items-start gap-3">
        <Icon
          ...
          class="text-amber-600 dark:text-amber-400 text-lg shrink-0 mt-0.5"
        />
        <div class="flex-1 min-w-0">
          <p
            class="text-sm font-medium text-amber-800 dark:text-amber-300 mb-1"
          >
            重要：请确保已配置环境变量
          </p>
          <p class="text-xs text-amber-700 dark:text-amber-400">
            应用绑定和消息推送需要访问 KV 存储。请在 EdgeOne 项目设置中配置：
            <code
              class="mx-1 px-1.5 py-0.5 bg-amber-100 dark:bg-amber-900/40 rounded text-[11px] font-mono"
            >
    ...
    <div class="flex-1 flex min-h-0">
      ...
      <template v-if="!isMobile">
        <!-- List Panel -->
        <div
          class="w-80 border-r border-gray-200 dark:border-gray-800 flex flex-col bg-white dark:bg-gray-900"
        >
      ...
      >
        <div class="min-h-screen flex items-center justify-center p-4">
          <div
            class="flex flex-col bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 shadow-lg rounded-xl w-full max-w-md"
          >
            <div
              class="flex justify-between items-center py-3 px-4 border-b border-gray-200 dark:border-gray-800"
            >
            ...
            <div class="p-4">
              <div class="space-y-4">
                ...
                <div
                  v-if="createForm.messageType === 'template'"
                  class="p-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg border border-yellow-200 dark:border-yellow-800"
                >
                  <div class="flex items-start gap-2 mb-2">
                    <Icon
                      icon="heroicons:light-bulb"
                      class="text-yellow-600 dark:text-yellow-400 text-lg shrink-0 mt-0.5"
                    />
                    <div class="flex-1">
                      <div
                        class="text-sm font-medium text-yellow-800 dark:text-yellow-300 mb-1"
                      >
                        模板消息配置要求
                      </div>
                      <div
                        class="text-xs text-yellow-700 dark:text-yellow-400 mb-2"
                      >
                        使用模板消息需要先在微信公众平台创建模板。推荐使用以下格式（可突破48小时限制）：
                      </div>
                      <pre
                        class="text-xs bg-yellow-100 dark:bg-yellow-900/30 px-2 py-1.5 rounded mb-2 overflow-x-auto"
                        v-pre
                      >
标题：{{ first.DATA }}
内容：{{ keyword1.DATA }}
备注：{{ remark.DATA }}</pre
                      >
                      <div
                        class="text-xs text-yellow-700 dark:text-yellow-400 mb-1 font-medium"
                      >
                        Webhook 参数映射：
                      </div>
                      <ul
                        class="text-xs text-yellow-700 dark:text-yellow-400 space-y-0.5"
                      >
                        <li>
                          •
                          <code
                            class="bg-yellow-100 dark:bg-yellow-900/30 px-1 rounded"
                            >title</code
                          >
                          →
                          <code
                ...
                <div v-if="createForm.messageType === 'template'">
                  ...
                  <input
                    v-model="templateId"
                    placeholder="微信模板消息 ID"
                    class="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none"
                  />
                  <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                    在微信公众平台创建模板后获得的模板 ID
                  </p>
                </div>
                <div
                  class="p-4 rounded-lg border bg-blue-50 border-blue-200 text-blue-800 dark:bg-blue-900/20 dark:border-blue-800 dark:text-blue-300"
                >
                  <div class="flex items-start gap-3">
                    <Icon
            ...
            <div
              class="flex justify-end gap-2 py-3 px-4 border-t border-gray-200 dark:border-gray-800"
            >
              <button
...
const selectedChannel = computed(() =>
  selectedApp.value
    ? channels.value.find((c) => c.id === selectedApp.value?.channelId)
    : null,
);
...
const messageTypeOptions = [
  {
    ...
    description:
      "客服消息，48小时内可推送。建议用户打开公众号定位推送以保持互动。",
  },
  {
    label: "模板消息",
    value: MessageTypes.TEMPLATE,
    description:
      "无48小时限制。正式公众号已停止新申请，推荐使用测试号自定义模板。",
    templateGuide: {
      title: "模板消息配置要求",
...
备注：{{remark.DATA}}`,
      fieldMapping: [
        ...
        {
          templateField: "keyword1",
          webhookParam: "desp",
          description: "消息内容",
        },
        {
          templateField: "remark",
          webhookParam: "(自动)",
          description: "备注信息",
        },
      ],
...
async function handleCreate() {
  ...
  if (
    createForm.value.messageType === MessageTypes.TEMPLATE &&
